...
function HighlightLink(link, linkInfo)
{
	...
	if ((linkInfo.status == LCR_DANGER) || (linkInfo.status == LCR_PHISHING) || (linkInfo.status == LCR_MALWARE))
		statusIcon.innerHTML = DangerImageHtml;
	else
	if (linkInfo.status == LCR_SUSPICIOUS)
...
function ShowInfoWindow(url, status, source, categories, x, y)
{
	try
	{
		var request = new Object();
	...
	try
	{
		...
		if (event.type == "mouseover" || event.type == "mousemove")
		{
				...
				event.target.attributes.getNamedItem(HightlightedAttrCategories))
			{
				...
				TimerId = window.setTimeout(ShowInfoWindow, ShowInfoWindowTimeout, url, status, source, categories, event.screenX, event.screenY);
			}
		}
		else
