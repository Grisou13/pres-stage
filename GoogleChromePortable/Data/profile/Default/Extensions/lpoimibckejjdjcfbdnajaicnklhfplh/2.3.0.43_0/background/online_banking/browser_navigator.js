function BrowserNavigator(plugin)
{
	...
	var that = this;
	var m_plugin = plugin;
	var m_cookiesHeadersParser = new KasperskyLabOnlineBanking.CookiesHeadersParser();
	var m_headersReplacer = new KasperskyLabOnlineBanking.HeadersReplacer();

	m_plugin.registerMethod('ob.navigateGet', function(args){
		that.navigateGet(args[0], args[1], args[2], args[3]);
	});
	
	m_plugin.registerMethod('ob.navigatePost', function(args){
		that.navigatePost(args[0], args[1], args[2], args[3], args[4], args[5]);
	});
	...
	function navigate(tabId, url, headersString, method, target, postData, postDataEncoding)
	{
		...
		trySetCookiesToBrowserFromHeaders(url, headers, m_cookiesHeadersParser);
